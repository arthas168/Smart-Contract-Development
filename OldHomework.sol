...
contract MemberVoter is Ownable, Destructible {
    ...
    modifier canRemoveMember(address adr) {
        ...
        require(members[adr].adr != address(0)); //member should exist
        _;
    }
    
    modifier canAddMember(address adr) {
        require(adr != address(0)); //the address should exist
        require(members[adr].adr == address(0)); //member shouldn't nexist
        _;
    }
    ...
    function _addMember(address adr) private canAddMember(adr) {
        assert(members[adr].adr == address(0)); //the person shouldn't be a member
    ...
    function _removeMember(address adr) private canRemoveMember(adr) { //private method that removes a member
        assert(members[adr].adr != address(0)); //the person should be a member
    ...
    function _addVoting(address proposedMember) private canAddMember(proposedMember) returns (bytes32) {
        bytes32 id = keccak256(abi.encodePacked(proposedMember, now)); //the ID of a vote is the unique hash of the member and the current time 
    ...
    function vote(bytes32 id, bool voteFor) public onlyMember {
        require(votings[id].proposedMember != address(0)); //the voting should exist
